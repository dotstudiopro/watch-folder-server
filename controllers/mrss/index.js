...
const getFeedData = (req, callback) => {
	...
	const bucketPromise = new AWS.S3({apiVersion: '2006-03-01'}).listObjectsV2(params).promise();
		...
		.then(async data => {
			...
			if (dataHolder.IsTruncated) {
				while (truncated) {
					...
					const nextPromise = new AWS.S3({apiVersion: '2006-03-01'}).listObjectsV2(params).promise();
					const nextData = await nextPromise.catch(err => {throw new Error(err)});
		    ...
		    console.log({totalNum: objects.length})
			    ...
			    const json = {
		    		"item": [
			    		...
			    		},
			    		{
			    			"name": "media:content",
			    			"attrs": {
			    				"url": encodeURI(base_aws_url + bucketName + "/" + obj.Key),
			    				"fileSize": obj.Size,
			    			},
			    			"children": [
			    				{
